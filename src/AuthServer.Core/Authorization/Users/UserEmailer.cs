...
namespace AuthServer.Authorization.Users
{
    ...
    /// </summary>
    public class UserEmailer : AuthServerServiceBase, IUserEmailer, ITransientDependency
    {
        ...
        public async Task SendPasswordResetLinkAsync(User user, string link = null)
        {
            ...
            if (!link.IsNullOrEmpty())
            {
                ...
                var expireDate = Uri.EscapeDataString(Clock.Now.AddHours(expirationHours)
                    .ToString(AuthServerConsts.DateTimeOffsetFormat, CultureInfo.InvariantCulture));
