...
namespace Inventory.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : InventoryAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<InventoryDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{InventoryConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{InventoryConsts.ConnectionStringName}", constring);
        }
