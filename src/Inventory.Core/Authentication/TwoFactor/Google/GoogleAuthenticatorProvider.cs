...
namespace Inventory.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : InventoryServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("Inventory", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
