...
using Gmobile.Migrations.Seed.Host;
...
namespace Gmobile.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : GmobileAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<GmobileDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{GmobileConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{GmobileConsts.ConnectionStringName}", constring);
        }
